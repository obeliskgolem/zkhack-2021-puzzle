...
pub fn kzg_commit(p: &DensePolynomial<Fr>, setup: &Vec<G1Affine>) -> G1Affine {
    p.coeffs()
        ...
        .map(|(c, p)| {
            if *c == Fr::zero() {
                G1Affine::zero().into_projective()
            } else {
                p.into_projective().mul(c.into_repr())
            }
        })
        .sum::<G1Projective>()
        .into_affine()
...
pub fn generate_accts(n: usize) -> Vec<Vec<Fr>> {
    (0..n)
        .into_iter()
